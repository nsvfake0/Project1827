...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; touch-action: manipulation; }
        ...
        .loader-text {
            color: var(--lux-gold); font-family: var(--font-head); font-size: 1.5rem; letter-spacing: 5px;
        ...
        .loader-progress {
            position: absolute; left: 0; top: 0; bottom: 0; width: 50%; background: var(--lux-gold);
            animation: loadAnim 2s infinite linear;
        }
        @keyframes loadAnim { 0% { left: -50%; } 100% { left: 100%; } }

        /* --- SCENES --- */
        #intro-scene, #hallway-scene, #final-scene { position: fixed; inset: 0; width: 100%; height: 100%; }
        
        #intro-scene { 
            z-index: 100; 
            background: var(--lux-black); 
            display: flex; justify-content: center; align-items: center; 
            opacity: 0; transition: opacity 1.5s ease; /* Hidden by default until loaded */
        }
        ...
        .door-card {
            width: 100%;
            height: 130px;
            background: rgba(20, 20, 20, 0.7); /* Darker, less blur for performance */
            border: 1px solid rgba(207, 161, 86, 0.3);
        ...
        .modal-content {
            background-color: var(--lux-brown);
        ...
        /* --- BUTTONS --- */
        .btn-action, .start-btn, .back-hallway-btn, #success-exit-btn {
        ...
        .start-btn {
             box-shadow: 0 0 15px rgba(207,161,86,0.3);
        ...
        /* --- SNAKE --- */
        #snake-canvas { background: #000; border: 1px solid var(--lux-gold); margin: 0 auto; display: block; max-width: 100%; }
        ...
        #final-scene { 
            ...
            background: #111; /* Removed gradient animation for performance */
            background: radial-gradient(circle at center, #2c1b18 0%, #000000 100%);
            color: var(--lux-gold); 
            text-align: center; 
        ...
        .final-title {
            ...
            margin-bottom: 20px;
            color: var(--lux-white);
            text-shadow: 0 0 10px rgba(207,161,86,0.5);
            letter-spacing: 3px;
        }
        ...
        .particle {
            position: absolute; border-radius: 50%; background: var(--lux-gold);
            box-shadow: 0 0 2px var(--lux-gold); animation: floatParticle 20s infinite linear; z-index: 201; opacity: 0.4;
        }
        ...
        .floating-heart {
            position: absolute; bottom: -50px; font-size: 20px; color: #8a0a0a; 
        ...
        #passcode-input {
            background: transparent; border: none; border-bottom: 1px solid var(--lux-gold);
            color: var(--lux-gold); font-family: var(--font-head); font-size: 2rem; width: 200px; text-align: center; margin-bottom: 20px; letter-spacing: 5px;
        }
...
<body>
    ...
    <audio id="door1-audio" preload="auto"><source src="door1.mp3" type="audio/mp3"></audio>
    ...
    <audio id="room6-audio" preload="auto"><source src="room6.mp3" type="audio/mp3"></audio>

    <div id="intro-scene">
        <button id="dev-btn" onclick="activateDevMode()">π</button>
        <video id="intro-video" muted playsinline webkit-playsinline loop preload="auto"><source src="intro.mp4" type="video/mp4"></video>
    ...
    <div id="hallway-scene">
        ...
        <div class="hallway-content">
            <h2>The Gallery</h2>
            <div class="door-grid">
                <div class="door-card" onclick="openDoor(1)"><div class="door-status" id="status-1">🧩</div></div>
                <div class="door-card" onclick="openDoor(2)"><div class="door-status" id="status-2">🐍</div></div>
                <div class="door-card" onclick="openDoor(3)"><div class="door-status" id="status-3">🎥</div></div>
                <div class="door-card" onclick="openDoor(4)"><div class="door-status" id="status-4">IV</div></div>
                <div id="final-door-wrapper">
                    <div class="door-card door-locked" id="final-door" onclick="tryFinalDoor()" style="max-width:200px;"><div class="door-status">🔒</div></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="modal-1" class="modal">
        <div class="modal-content">
            <span class="close-btn" id="close-1" onclick="closeModal()" style="display:none;">×</span>
            <h2>Fragments</h2>
    ...
    <div id="modal-2" class="modal">
        <div class="modal-content">
            <span class="close-btn" id="close-2" onclick="closeModal()" style="display:none;">×</span>
    ...
    <div id="modal-3" class="modal">
        <div class="modal-content">
            <span class="close-btn" id="close-3" onclick="closeModal()">×</span>
            <h2>Memory Lane</h2>
    ...
    <div id="modal-generic" class="modal">
        <div class="modal-content">
            <span class="close-btn" style="display:block;" onclick="closeModal()">×</span>
            <h2 id="gen-title"></h2>
            <div id="gen-body"></div>
            <button class="btn-action" onclick="closeModal()">Return</button>
        </div>
    ...
    <script>
        ...
        window.onload = function() {
            ...
            }, 2000); // Forced 2s wait to ensure smoother playback
        };
        ...
        function activateDevMode() {
            ...
            if(code === "1827") {
                ...
                for(let i=1; i<=4; i++) document.getElementById(`status-${i}`).innerText = "✨";
        ...
        function createFloatingHearts() {
            ...
            }, 1800); // SLOWED DOWN FOR PERFORMANCE
        }
